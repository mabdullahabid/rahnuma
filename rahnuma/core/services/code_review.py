...
def code_review(repo, pr):
    options = Options(
        review_simple_changes=False,
        openai_light_model="gpt-3.5-turbo-0125",
        path_filters=["!package-lock.json", "!package.json"],
    )
    ...
    inputs.description = pr.body or "No description provided"
    ...
    for file in files:
        if options.path_filters.check(file.filename):  # Use checkPath to filter files
    ...
    return summarize_final_response
